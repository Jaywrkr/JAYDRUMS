...
3. **Práctica**: motor de juego con carriles por pieza. Trae un mini-currículo de 4 niveles cortos (10-20s) pensado para ir de cero a un groove completo, un elemento nuevo por nivel, con estrellas (1-3, según precisión) y desbloqueo progresivo: hay que sacar al menos 1 estrella en un nivel para destrabar el siguiente. Las notas viajan sincronizadas al patrón, en vertical (arriba→abajo) u horizontal (derecha→izquierda), a elección. Cuenta regresiva y metrónomo audible (Web Audio API) para no perder el tiempo. Control de velocidad (50%–150% del BPM original) para practicar lento y subir de a poco. Se evalúan contra los golpes reales usando el mapa calibrado (perfecto ±30 ms, bien ±70 ms, flojo ±150 ms) y el offset de latencia medido en la pestaña Latencia. Combo con multiplicador de puntaje, popups de "¡Perfecto!/Bien/Flojo/Fallo", pantalla de resultados con estrellas y mejor puntaje guardado por lección (`localStorage`).
